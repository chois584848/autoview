...
function renderTopbar() {
  ...
  if (state.view === 'editor') {
    ...
    const extra = ro ? `<span class="shared-badge">${ic('eye', 14)} Read-only · Shared by Emily</span>` : '';
  ...
  const rightPane = pMin
    ...
    : `<div class="editor-pane right">
        <div class="pane-head">
          ...
          <div class="ph-right">
            ${state.sharedReadonly ? '' : `<button class="icon-btn" data-action="export-menu" title="Export & share">${ic('export', 16)}</button>`}
...
// component-level share → same team-share popup as projects
function showShare() {
  const name = state.componentName || 'Component';
  const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
  showTeamShare(name, `Invite teammates to collaborate on “${escapeHtml(name)}”.`, `www.autoview.app/c/${slug}`);
}
...
function showShareProject(pi) {
  const proj = projects[pi];
  if (!proj) return;
  const slug = proj.name.toLowerCase().replace(/\s+/g, '-');
  showTeamShare(proj.name, `Invite teammates to collaborate on “${escapeHtml(proj.name)}” and its components.`, `www.autoview.app/p/${slug}`);
}

function showTeamShare(shareName, desc, link) {
  const avatarColors = ['#6366f1', '#0891b2', '#db2777', '#65a30d', '#9333ea'];
  const ava = (name, i) => `<span class="sp-ava" style="background:${avatarColors[i % avatarColors.length]}">${escapeHtml(name.trim()[0] || '?').toUpperCase()}</span>`;
  const team = [
    { name: state.user.name, sub: state.user.email, role: 'Owner', owner: true },
    { name: 'Jordan Kim', sub: 'jordan@autoview.app', role: 'Can edit' },
    { name: 'Alex Park', sub: 'alex@autoview.app', role: 'Can view' },
  ];
  const ddPill = (type, val, cls = '') => `<button class="dd-pill ${cls}" data-dd="${type}" data-val="${escapeHtml(val)}"><span class="dd-val">${escapeHtml(val)}</span>${ic('chevron', 14)}</button>`;
  const roleControl = (role, owner) => owner
    ? `<span class="sp-role owner">Owner</span>`
    : ddPill('role', role, 'sp-role');
  const people = team.map((p, i) => `
    <div class="sp-person">
      ${ava(p.name, i)}
      <div class="sp-meta"><div class="sp-name">${escapeHtml(p.name)}${p.owner ? ' (you)' : ''}</div><div class="sp-sub">${escapeHtml(p.sub)}</div></div>
      ${roleControl(p.role, p.owner)}
    </div>`).join('');
    ...
    <div class="overlay" data-overlay>
      <div class="modal wide">
        ...
        <div class="invite-row">
          <input id="invite-email" type="email" placeholder="Add people by email…" />
          ${ddPill('invite', 'Can edit', 'sp-role')}
          <button class="btn-primary btn" data-action="invite-member">Invite</button>
        </div>
        <div class="sp-label">People with access</div>
        <div class="sp-list" id="sp-list">${people}</div>
        <div class="divider-h"></div>
        <div class="sp-access">
          <span class="sp-access-ic">${ic('link', 16)}</span>
          <div class="sp-meta"><div class="sp-name">General access</div><div class="sp-sub" id="sp-access-sub">Restricted — only invited people</div></div>
          ${ddPill('access', 'Restricted', 'sp-access-dd')}
        </div>
  ...
  const wirePill = (pill) => {
    pill.addEventListener('click', (e) => {
      e.stopPropagation();
      const type = pill.dataset.dd, cur = pill.dataset.val;
      const opts = type === 'access' ? ['Restricted', 'Anyone with the link']
        : type === 'invite' ? ['Can edit', 'Can view']
          : ['Can edit', 'Can view', 'Remove'];
      const w = type === 'access' ? 210 : 160;
      const rect = pill.getBoundingClientRect();
      const items = opts.map(o => ({
        id: o, label: o, danger: o === 'Remove',
        icon: o === cur ? ic('check', 15) : '',
        action: () => {
          if (o === 'Remove') { const row = pill.closest('.sp-person'); if (row) row.remove(); toast('Access removed'); return; }
          pill.dataset.val = o; pill.querySelector('.dd-val').textContent = o;
          if (type === 'access') { const sub = $('#sp-access-sub'); if (sub) sub.textContent = o === 'Restricted' ? 'Restricted — only invited people' : 'Anyone with the link can view'; }
        },
      }));
      showCtxMenu(rect.right - w, rect.bottom + 5, items, { minWidth: w });
    });
  };
  modalRoot.querySelectorAll('.dd-pill').forEach(wirePill);

  const emailInput = $('#invite-email');
  const addMember = () => {
    const email = (emailInput.value || '').trim();
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) { toast('Enter a valid email', '!'); return; }
    const role = modalRoot.querySelector('[data-dd="invite"]').dataset.val;
    const name = email.split('@')[0].replace(/[._-]+/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    const row = document.createElement('div');
    row.className = 'sp-person';
    row.innerHTML = `${ava(name, $('#sp-list').children.length)}<div class="sp-meta"><div class="sp-name">${escapeHtml(name)}</div><div class="sp-sub">${escapeHtml(email)}</div></div>${ddPill('role', role, 'sp-role')}`;
    $('#sp-list').appendChild(row);
    wirePill(row.querySelector('.dd-pill'));
    emailInput.value = '';
    toast(`Invited ${email}`);
  };
  if (emailInput) {
    emailInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); addMember(); } });
    $('[data-action="invite-member"]').addEventListener('click', addMember);
  }
}
...
function showCtxMenu(x, y, items, opts = {}) {
  ...
  el.style.left = Math.max(8, Math.min(x, window.innerWidth - (opts.minWidth || 220) - 8)) + 'px';
  el.style.top = y + 'px';
...
function exportMenu(x, y) {
  showCtxMenu(x, y, [
    ...
    { divider: true },
    { id: 'share', icon: ic('share'), label: 'Share', action: () => showShare() },
  ], { minWidth: 230 });
}
  ...
  document.querySelectorAll('[data-project-menu]').forEach(el => {
    el.addEventListener('click', (e) => {
      ...
      showCtxMenu(r.left, r.bottom + 4, [
        ...
        { id: 'share', icon: ic('share'), label: 'Share', action: () => showShareProject(pi) },
